...
public class Bootstrap implements BladeLoader {
    ...
    public void preLoad(Blade blade) {
        ...
        this.s3AccessKey = blade.environment().getOrNull("aws.access_key");
        this.s3SecretKey = blade.environment().getOrNull("aws.secret_key");


        final String dbHost = blade.environment().getOrNull("app.database.host");
        final String dbPort = blade.environment().getOrNull("app.database.port");
        final String dbDatabase = blade.environment().getOrNull("app.database.database");
        final String dbUserName = blade.environment().getOrNull("app.database.username");
        final String dbPassword = blade.environment().getOrNull("app.database.password");
...
//        Anima.open("jdbc:mysql://" + dbHost + ":" + dbPort + "/" + dbDatabase+"", dbUserName, dbPassword);
        Anima.open("jdbc:mysql://" + dbHost + ":" + dbPort + "/" + dbDatabase+"?useUnicode=true&characterEncoding=UTF8", dbUserName, dbPassword);
