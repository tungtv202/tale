...
public class SiteService {
    ...
    public void initSite(Users users) {
        ...
        try {
            ...
            new Logs("init site", null, "", uid).save();
        } catch (Exception e) {
    ...
    public List<Archive> getArchives() {
        String sql =
            "select DATE_FORMAT(created, '%Y-%m')  as date_str, count(*) as count  from t_contents "
                +
        ...
        if (null != archives) {
            List<Archive> result = archives.stream()
    ...
    private Archive parseArchive(Archive archive) {
        ...
        Date sd = DateKit.toDate(dateStr + "-01", "yyyy-MM-dd");
        archive.setDate(sd);
        ...
        String sql =
                "select *  from t_contents Where 1=1 ";
        sql += " AND type = '" + Types.ARTICLE + "'";
        sql += " AND status = '" + Types.PUBLISH + "'";
        sql += " AND created >= '" + dateFormat.format(sd) + "'";
        sql += " AND created <= '" + dateFormat.format(endSd) + "'";
        List<Contents> contents = select().bySQL(Contents.class, sql).all();
